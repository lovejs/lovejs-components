import { ConfigLoader, ConfigToken as CT, ConfigTag, ConfigError } from "../..";
import { ConfigNormalizer, ConfigTagList } from "../ConfigLoader";
import { ConfigLoaderExtension } from "../ConfigLoaderExtension";
import { ConfigToken } from "../ConfigToken";

const getTag = (name: string, schema?: object): { [tag: string]: ConfigTag } => ({
    [name]: {
...
describe("#Config [Loader] - Normalize Tags", function() {
    it("should resolve simple Config Token", async () => {
        ...
        const loader = new ConfigLoader({ tags: { ...getTag("tag1") } });
    ...
    it("should resolve nested Config Tokens", async () => {
        ...
        const loader = new ConfigLoader({ tags: { ...getTag("tag1"), ...getTag("tag2"), ...getTag("tag3") } });
        const config = await loader.loadConfig(configuration);
...
describe("#Config [Loader] - Normalizers", function() {
    it("should use simple normalizer", async () => {
        ...
        const normalizers: ConfigNormalizer[] = [
            { path: "a", normalize: async data => data.join(",") },
            { path: "b", normalize: async data => 123 }
        ];

        const loader = new ConfigLoader({ normalizers });
        const config = await loader.loadConfig(configuration);
...
describe("#Config [Loader] - Extensions", function() {
    it("should accept extension in constructor", async () => {
        const extension: ConfigLoaderExtension = {
            async getTags() {
                return {
                    tag_extension: {
                        schema: {},
        ...
        const loader = new ConfigLoader({ extensions: [extension] });
        const config = await loader.loadConfig({
            a: new ConfigToken("tag_extension"),
            b: "test"
        });
